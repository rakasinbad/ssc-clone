...
import { AssociatedPortfolioActions, associationFailureActionNames } from '../actions';
...
export class AssociatedPortfoliosEffects {
            ...
            this.actions$.pipe(
                ...
                tap(() => {
                    ...
                    this.associationStore.dispatch(
                        AssociatedPortfolioActions.clearAssociatedPortfolios()
                    );

                    // Hanya memunculkan notifikasi jika memang ada store yang terhapus.
                    this.notice.open('All selected portfolios have been cleared.', 'info', {
                ...
                if (!authState) {
                    return this.helper$.decodeUserToken().pipe(
                        ...
                        switchMap<[User, IQueryParams], Observable<AnyAction>>(
                            this.processAssociatedPortfoliosRequest
                        ),
                        catchError(err =>
                            this.sendErrorToState(err, 'fetchAssociatedPortfoliosFailure')
                        )
                    );
                } else {
    ...
    processAssociatedPortfoliosRequest = ([userData, queryParams]: [
        User,
        IQueryParams
        ...
        if (queryParams['combined']) {
            return forkJoin([
                this.associatedPortfolioService
                    .findPortfolio<IPaginatedResponse<Portfolio>>({
                        ...queryParams,
                        supplierId,
                        type: 'group'
                    } as IQueryParams)
                ...
                switchMap(responses => {
                    ...
                    if (queryParams.paginate) {
                        if (responses.group) {
                            const newResponseGroup = responses.group as IPaginatedResponse<
                                Portfolio
                            >;
                        ...
                        if (responses.direct) {
                            const newResponseDirect = responses.group as IPaginatedResponse<
                                Portfolio
                            >;
                        ...
                        return of(
                            AssociatedPortfolioActions.fetchAssociatedPortfoliosSuccess({
                                payload: {
                                    data: newResponse.map(
                                        portfolio =>
                                            new Portfolio({
                                                ...portfolio,
                                                source: queryParams['fromSalesRep']
                                                    ? 'list'
                                                    : 'fetch',
                                                storeQty:
                                    ...
                                    payload: {
                                        data: response.data.map(
                                            portfolio =>
                                                new Portfolio({
                                                    ...portfolio,
                                                    source: newQuery['fromSalesRep']
                                                        ? 'list'
                                                        : 'fetch',
                                                    storeQty:
                                                        portfolio.storeQty ||
                                                        portfolio['storeAmount'],
    ...
    sendErrorToState = (
        ...
        if (err instanceof ErrorHandler) {
            return of(
                AssociatedPortfolioActions[dispatchTo]({
                    payload: err
                })
            );
        }
