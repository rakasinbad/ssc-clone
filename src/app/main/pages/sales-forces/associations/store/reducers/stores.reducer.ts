...
export const reducer = createReducer(
    initialState,
    on(StoreActions.fetchStoreRequest, StoreActions.fetchStoresRequest, state => ({
        ...state,
        isLoading: true
    })),
    on(StoreActions.fetchStoreFailure, StoreActions.fetchStoresFailure, state => ({
        ...state,
        isLoading: false
    })),
    ...
    on(StoreActions.markStoreAsRemoved, (state, { payload }) => {
    ...
    on(StoreActions.markStoresAsRemoved, (state, { payload }) => {
    ...
    on(StoreActions.abortStoreAsRemoved, (state, { payload }) => {
        const newStore = new Store(state.entities[payload]);
        newStore.setDeletedAt = null;

        return adapter.upsertOne(newStore, state);
    }),
    on(StoreActions.abortStoresAsRemoved, (state, { payload }) => {
        const newStore: Array<Store> = [];
    ...
    on(StoreActions.truncateStores, state =>
        adapter.removeAll({
            ...state,
            total: 0
        })
