...
@Component({
    selector: 'app-sales-repsv2',
    templateUrl: './sales-repsv2.component.html',
    styleUrls: ['./sales-repsv2.component.scss'],
})
    ...
    ngOnInit(): void {
        ...
        this.storage.get('user').subscribe((data: any) => {
            const safeUrl = this.domSanitizer.bypassSecurityTrustResourceUrl(
                `${environment.microSiteHost}/salesmanagement/salesrep?token=${data.token}`
            );
