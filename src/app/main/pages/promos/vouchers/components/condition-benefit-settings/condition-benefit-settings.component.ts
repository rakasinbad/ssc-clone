...
export class VoucherTriggerConditionBenefitSettingsComponent
    ...
    private patchConditionBenefitForm(source: Voucher, index: number): void {
        ...
        let patchValue: any = {
            ...
            condition: {
                base:
                    source.promoConditions[index].conditionBase === 'value'
                        ? 'order-value'
                        : source.promoConditions[index].conditionBase,
        ...
        if (this.isViewMode()) {
            this.form
                .get(['conditionBenefit', index, 'condition', 'base'])
                .disable({ onlySelf: true, emitEvent: false });
            this.form
                .get(['conditionBenefit', index, 'benefit', 'base'])
                .disable({ onlySelf: true, emitEvent: false });
            this.form
                .get(['conditionBenefit', index, 'benefit', 'qty', 'applySameSku'])
                .disable({ onlySelf: true, emitEvent: false });
            this.form
                .get(['conditionBenefit', index, 'benefit', 'qty', 'multiplicationOnly'])
                .disable({ onlySelf: true, emitEvent: false });
        } else {
            this.form
                .get(['conditionBenefit', index, 'condition', 'base'])
                .enable({ onlySelf: true, emitEvent: true });
            this.form
                .get(['conditionBenefit', index, 'benefit', 'base'])
                .enable({ onlySelf: true, emitEvent: true });
            this.form
        ...
        combineLatest([this.trigger$, this.store.select(VoucherSelectors.getSelectedVoucher)])
            ...
            .subscribe(({ voucher, userSupplier }) => {
                ...
                this.form.patchValue({
                    id: voucher.id,
                    calculationMechanism: voucher.isComulative ? 'cummulative' : 'non-cummulative',
                });

                if (this.isViewMode()) {
                    this.form
                        .get('calculationMechanism')
                        .disable({ onlySelf: true, emitEvent: false });
                } else {
                    this.form
                        .get('calculationMechanism')
                ...
                for (const [idx, _] of voucher.promoConditions.entries()) {
                    this.patchConditionBenefitForm(voucher, idx);
                }
    ...
    checkRebate(index: number): AsyncValidatorFn {
        const check = () => {
            ...
            const conditionValue = +this.form.get(['conditionBenefit', index, 'condition', 'value'])
                .value;
            const cashRebate = +this.form.get([
                'conditionBenefit',
                index,
                'benefit',
                'cash',
    ...
    addConditionBenefitForm(): void {
        ...
        if ((this.form.get('conditionBenefit') as FormArray).length > 1 && !this.isViewMode()) {
            const multiplication = this.form.get([
                'conditionBenefit',
                0,
                'benefit',
                'qty',
        ...
        lastControl.valueChanges
            .pipe(
                distinctUntilChanged(),
                debounceTime(100),
                tap((value) =>
                    HelperService.debug('CONDITION BENEFITS VALUE CHANGED', {
                        index: lastIdx,
                        value,
                        control: lastControl,
                    })
            ...
            .subscribe({
                ...
                complete: () =>
                    HelperService.debug('CONDITION BENEFITS VALUE CHANGES COMPLETED', {
                        index: lastIdx,
                        control: lastControl,
                    }),
            });
        ...
        setTimeout(() => {
            if (lastIdx > 0) {
                const previousControl = this.form.get([
                    'conditionBenefit',
                    lastIdx - 1,
                ]) as FormGroup;
                ...
                if (previousBenefitBase === 'qty') {
                    ...
                    lastControl.get('benefit.cash').disable({ onlySelf: true, emitEvent: false });
                    const previousBonusPercentDiscount = +previousControl.get(
                        'benefit.percent.percentDiscount'
                    ).value;
                    const previousBonusMaxRebate = +previousControl.get('benefit.percent.maxRebate')
                        .value;

                    // BONUS % DISCOUNT
                    lastControl
                        .get('benefit.percent.percentDiscount')
                        .setValue(previousBonusPercentDiscount - 1);
                    ...
                    // BONUS MAX REBATE
                    lastControl
                        .get('benefit.percent.maxRebate')
                        .setValue(previousBonusMaxRebate - 1);
                    ...
                    lastControl
                        .get('benefit.cash.rebate')
                        .setAsyncValidators([this.checkRebate(lastIdx)]);
    ...
    private initFormCheck(): void {
        (this.form.statusChanges as Observable<FormStatus>)
            .pipe(
                ...
                tap((value) =>
                    HelperService.debug(
                        'PERIOD TARGET PROMO CONDITON & BENEFIT SETTINGS FORM STATUS CHANGED:',
                        value
                    )
                ),
                // filter(value => {
        ...
        this.form.valueChanges
            .pipe(
                ...
                tap((value) =>
                    HelperService.debug(
                        '[BEFORE MAP] PERIOD TARGET PROMO CONDITON & BENEFIT SETTINGS FORM VALUE CHANGED',
                        value
                    )
                ),
                map(() => {
                    ...
                    return {
                        ...rawValue,
                        conditionBenefit: rawValue.conditionBenefit.map((data) => ({
                ...
                tap((value) =>
                    HelperService.debug(
                        '[AFTER MAP] PERIOD TARGET PROMO CONDITON & BENEFIT SETTINGS FORM VALUE CHANGED',
                        value
                    )
                ),
                takeUntil(this.subs$)
        ...
        (this.form.get(['conditionBenefit', 0, 'benefit', 'qty', 'applySameSku'])
            ...
            .pipe(
                ...
                tap((value) =>
                    HelperService.debug(
                        'PERIOD TARGET PROMO CONDITON & BENEFIT SETTINGS APPLY SAME SKU CHANGED:',
                        value
                    )
                ),
                takeUntil(this.subs$)
