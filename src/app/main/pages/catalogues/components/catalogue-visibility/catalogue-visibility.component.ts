            ...
            this.form.valueChanges
                .pipe(
                    ...
                    map((value) => {
                        const formValue = {
                            status: value.status,
                            platformVisibility: !value.platformVisibility ? 'all' : value.platformVisibility,
    ...
    private _patchForm(): void {
        combineLatest([this.trigger$, this.catalogueFacade.catalogue$])
            ...
            .subscribe(({ catalogue: item }) => {
                ...
                const platformVisibilityCtrl = this.form.get('platformVisibility');
                const isBonusCtrl = this.form.get('isBonus');
                const isExclusiveCtrl = this.form.get('isExclusive');
                statusCtrl.setValue(item.status);
                platformVisibilityCtrl.setValue(!item.platformVisibility ? 'all': item.platformVisibility);
