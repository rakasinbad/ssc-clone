...
export class CataloguesService implements OnDestroy {
    ...
    private sanitizePrice(value: string): number {
    ...
    patchCatalogue(
        ...
        if ('retailBuyingPrice' in payload) {
            payload['retailBuyingPrice'] = this.sanitizePrice(String(payload['retailBuyingPrice']));
        }

        if ('discountedRetailBuyingPrice' in payload) {
            payload['discountedRetailBuyingPrice'] = this.sanitizePrice(
                String(payload['discountedRetailBuyingPrice'])
            );
        }
