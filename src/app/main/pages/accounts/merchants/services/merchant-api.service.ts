...
import { IStoreExist } from '../models/store-exist.model';
...
export class MerchantApiService {
    ...
    /**
     * API to Check External ID
     * 
     * @private
     * @memberof MerchantApiService
     */
    private readonly _checkExternalStoreExist = '/store-external-id';
    ...
    /**
     * Check is Store Already Exists or Not based on External ID
     * 
     * @param externalId 
     * @returns 
     */
    isStoreAlreadyExist(externalId: string): Observable<IStoreExist> {
        const safeExternalId = String(externalId).replace(/\//g, '');

        const url = this._$helper.handleApiRouter(
            `${this._checkExternalStoreExist}/${safeExternalId}`,
        )

        return this.http.get<IStoreExist>(url);
    }
