...
@Component({
    selector: 'app-merchant-setting-store-id-generation',
    templateUrl: './store-id-generation.component.html',
    styleUrls: ['./store-id-generation.component.scss'],
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class MerchantSettingStoreIdGenerationComponent implements OnInit, OnDestroy {
    form: FormGroup;
    ...
    constructor(
        ...
        this.isLoading$ = this.merchantStore
        ...
        this.form
            ...
            .subscribe((value: string) => {
                ...
                if (!value) {
                    this.form
                        .get('nextNumber')
                        .patchValue(String('').padStart(+maxDigit, '0'), optNextNumber);
                } else {
                    const nonZeroLeading = String(value).split(/^0+/);
                    ...
                    if (nonZeroLeading.length === 0) {
                        this.form
                            .get('nextNumber')
                            .patchValue(String('0').padStart(+maxDigit, '0'), optNextNumber);
                    } else if (nonZeroLeading.length === 1) {
                        ...
                        this.form
                            .get('nextNumber')
                            .patchValue(String(value).padStart(+maxDigit, '0'), optNextNumber);
                    } else {
                        this.form
                            .get('nextNumber')
                            .patchValue(
                                String(nonZeroLeading[1]).padStart(+maxDigit, '0'),
                                optNextNumber
                            );
                    }
                }
        ...
        combineLatest([
            ...
            .subscribe(([userSupplier, storeSettings]) => {
                ...
                if (storeSettings.length > 0) {
                    ...
                    this.merchantStore.dispatch(
                        StoreSettingActions.setSelectedStoreSettingId({ payload: storeSetting.id })
                    );
    ...
    onSubmit(): void {
        ...
        if (this.isEdit) {
            this.merchantStore.dispatch(
                StoreSettingActions.confirmUpdateStoreSetting({
                    payload: {
