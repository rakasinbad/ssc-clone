...
import { merchantFailureActionNames, StoreSettingActions } from '../actions';
...
export class StoreSettingEffects {
        ...
        () =>
            this.actions$.pipe(
                ...
                exhaustMap((payload: PayloadUpdateStoreSetting) => {
                    ...
                    return dialogRef.afterClosed().pipe(map((id: string) => [id, payload]));
                }),
                ...
                if (!authState) {
                    return this.helper$.decodeUserToken().pipe(
                        ...
                        switchMap<[User, IQueryParams], Observable<AnyAction>>(
                            this.processStoreSettingsRequest
                        ),
            ...
            switchMap(([storeSetting, authState]: [Partial<StoreSetting>, TNullable<Auth>]) => {
                ...
                if (!authState) {
                    return this.helper$.decodeUserToken().pipe(
                        ...
                        switchMap<[User, Partial<StoreSetting>], Observable<AnyAction>>(
                            this.processCreateStoreSettingsRequest
                        ),
            ...
            this.actions$.pipe(
                ...
                tap(error => {
                    ...
                    if (!error.id.startsWith('ERR_UNRECOGNIZED')) {
                        this.notice$.open(
                            `Failed to create a new store setting. Reason: ${error.errors}`,
                            'error',
                            noticeSetting
                        );
                    } else {
                        this.notice$.open(
                            `Something wrong with our web while creating a new store setting. Please contact Sinbad Team.`,
                            'error',
                            noticeSetting
                        );
                    }
            ...
            switchMap(([payload, authState]: [PayloadUpdateStoreSetting, TNullable<Auth>]) => {
                ...
                if (!authState) {
                    return this.helper$.decodeUserToken().pipe(
                        ...
                        switchMap<[User, PayloadUpdateStoreSetting], Observable<AnyAction>>(
                            this.processUpdateStoreSettingsRequest
                        ),
        ...
        () =>
            this.actions$.pipe(
                ...
                tap(error => {
                    ...
                    if (!error.id.startsWith('ERR_UNRECOGNIZED')) {
                        this.notice$.open(
                            `Failed to update a store setting. Reason: ${error.errors}`,
                            'error',
                            noticeSetting
                        );
                    } else {
                        this.notice$.open(
                            `Something wrong with our web while updating a store setting. Please contact Sinbad Team.`,
                            'error',
                            noticeSetting
                        );
                    }
    ...
    processCreateStoreSettingsRequest = ([_, storeSetting]: [
        User,
        Partial<StoreSetting>
    ]): Observable<AnyAction> => {
        return this.storeSettingApi.create(storeSetting).pipe(
            catchOffline(),
            switchMap(response => {
                return of(
                    StoreSettingActions.createStoreSettingSuccess({
                        payload: response
                    })
                );
            }),
    ...
    processUpdateStoreSettingsRequest = ([_, payload]: [
        User,
        PayloadUpdateStoreSetting
    ]): Observable<AnyAction> => {
        return this.storeSettingApi.patch<StoreSetting>(payload.body, payload.id).pipe(
            catchOffline(),
            switchMap(response => {
                return of(
                    StoreSettingActions.updateStoreSettingSuccess({
                        payload: response
                    })
                );
            }),
    ...
    processStoreSettingsRequest = ([userData, queryParams]: [User, IQueryParams]): Observable<
        AnyAction
    > => {
        ...
        return this.storeSettingApi
            .findAll<IPaginatedResponse<StoreSetting>>(
            ...
            .pipe(
                ...
                switchMap(response => {
                    if (newQuery.paginate) {
                        return of(
                            StoreSettingActions.fetchStoreSettingsSuccess({
                                payload: {
    ...
    sendErrorToState = (
        ...
        dispatchTo: merchantFailureActionNames
    ): Observable<AnyAction> => {
        if (err instanceof ErrorHandler) {
            return of(
                StoreSettingActions[dispatchTo]({
                    payload: err
                })
            );
        }
