...
export class MerchantEffects {
    ...
    createStoreFailure$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                map((action) => action.payload),
                // withLatestFrom(this.store.pipe(select(geStoreErrorById('createStoreFailure')))),
                tap((resp) => {
                    const msg =
                        resp && resp.errors && resp.errors.error && resp.errors.error.data
                            ? resp.errors.error.data
                ...
                tap((resp: any) => {
                    ...
                    payload.then((payload) => {
                        if (payload.code === 'group_pricing') {
                            this.router
                                .navigate([`/pages/account/stores/new/${resp.storeId}`], {
                                    replaceUrl: true,
                                })
                                .finally(() => {
                                    this._$notice.open(
                                        `${resp.name} berhasil ditambah`,
                                        'success',
                                        {
                                            verticalPosition: 'bottom',
                                            horizontalPosition: 'right',
    ...
    checkStoreExternalIdRequest$ = createEffect(() =>
        this.actions$.pipe(
            ofType(StoreActions.checkStoreExternalIdRequest),
            switchMap(({ externalId }) => {
                return this._$merchantApi.isStoreAlreadyExist(externalId).pipe(
                    map(resp => {
                        this._$log.generateGroup('RESPONSE REQUEST CHECK ID STORE', {
                            response: {
                                type: 'log',
                                value: resp,
                            },
                        ...
                        if (resp.exist) {
                            return StoreActions.checkStoreExternalIdFailure({
                                payload: {
                                    id: 'checkStoreExternalIdFailure',
                                    errors: 'External ID already exist',
                                },
                            });
                        }

                        return StoreActions.checkStoreExternalIdSuccess();
                    }),
                    catchError((err) => {
                        return of(
                            StoreActions.checkStoreExternalIdFailure({
                                payload: {
                                    id: 'checkStoreExternalIdFailure',
                                    errors: `${err.code || 'ERR'} ${err.message || 'Internal Server Error'}`,
                                },
                            })
                        );
                    }),
    ...
    /*
    // checkStoreExternalIdSuccess$ = createEffect(() => 
    //    this.actions$.pipe(
    //      ofType(StoreActions.checkStoreExternalIdSuccess),
    //      withLatestFrom(
    //          this.store.select(
    //              StoreSelectors.geStoreErrorById('checkStoreExternalIdFailure'),
    //          ),
    //      ),
    //      switchMap(([_, errors]) => {
    //          if (errors.length == 0) {
    //              return of(
    //                  FormActions.enableSaveButton()
    //              )
    //          }
    //      })
    //  )
    ...
    //checkStoreExternalIdFailure$ = createEffect(() => 
    //  this.actions$.pipe(
    //      ofType(StoreActions.checkStoreExternalIdFailure),
    //      forEach(_ => {
    //          return of(
    //              FormActions.disableSaveButton()
    //          )
    //      })
    //  )
    //);
