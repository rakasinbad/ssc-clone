...
import {
    CreditLimitGroup,
    CreditLimitStore,
} from 'app/main/pages/finances/credit-limit-balance/models';
...
import { StoreSegmentationTypesApiService as StoreSegmentationApiService } from 'app/shared/components/selection-tree/store-segmentation/services/store-segmentation-api.service';
...
import { geStoreErrorById } from '../store/selectors/merchant.selectors';
...
export class MerchantFormComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    searchSalesman: FormControl = new FormControl();
    ...
    isCheckingKtp$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
    ...
    sStoreIdsErrors$: Observable<string>;
    ...
    salesRepGroup$: Observable<Array<SalesRep>>;
    ...
    @ViewChild('salesSelect', { static: false }) salesSelect: MatSelect;
    @ViewChild('selectStoreType', { static: false })
    selectStoreType: TemplateRef<StoreSegmentationTreeComponent>;
    @ViewChild('selectStoreGroup', { static: false })
    selectStoreGroup: TemplateRef<StoreSegmentationTreeComponent>;
    @ViewChild('selectStoreChannel', { static: false })
    selectStoreChannel: TemplateRef<StoreSegmentationTreeComponent>;
    @ViewChild('selectStoreCluster', { static: false })
    selectStoreCluster: TemplateRef<StoreSegmentationTreeComponent>;
    ...
    constructor(
        ...
        private cataloguestore: Store<fromCatalogue.FeatureState>,
        ...
        this.cataloguestore.dispatch(CatalogueActions.fetchPricingSettingsRequest());
    ...
    ngOnInit(): void {
        ...
        this.cataloguestore
            ...
            .subscribe((payload) => {
                ...
                if (payload.code === 'group_pricing' && !this.isEditMode) {
                    this.footerConfig.action.save.label = 'Next';
                }
                this.store.dispatch(
                    UiActions.setFooterActionConfig({
                        payload: this.footerConfig,
                    })
                );
            });
        ...
        if (this.pageType === 'edit') {
            ...
            this.store
                .select(StoreSelectors.getSelectedSupplierStore)
                .pipe(
                    switchMap((supplierStore) => {
                        if (supplierStore) {
                            return forkJoin({
                                type: this._$segmentation.resolve(
                                    supplierStore.outerStore['storeType']['typeId'],
                                    'type'
                                ),
                                group: this._$segmentation.resolve(
                                    supplierStore.outerStore['storeGroup']['groupId'],
                                    'group'
                                ),
                                channel: this._$segmentation.resolve(
                                    supplierStore.outerStore['storeChannel']['channelId'],
                                    'channel'
                                ),
                                cluster: this._$segmentation.resolve(
                                    supplierStore.outerStore['storeCluster']['clusterId'],
                                    'cluster'
                                ),
                            }).pipe(
        ...
        // Get selector dropdown sales rep
        this.salesRepGroup$ = this.store.select(DropdownSelectors.getSalesRepDropdownState);
        // Fetch request sales rep
        this.store.dispatch(DropdownActions.fetchDropdownSalesRepRequest());
        ...
        this.store
            .select(StoreSelectors.geStoreErrorById('checkStoreExternalIdFailure'))
            .pipe(map((items) => items.map((item) => item.errors as string)))
            .subscribe((items) => {
                if (items.length > 0) {
        ...
        if (this.pageType !== 'edit') {
            this.form
                .get('storeInfo.address.warehouse')
                .valueChanges.pipe(takeUntil(this._unSubs$))
                .subscribe((v) => {
                    if (v) {
                        this.store.dispatch(
                            DropdownActions.fetchDropdownInvoiceGroupWhSupRequest({
                                warehouseId: v,
                            })
                        );
                        this.form.get('storeInfo.payment.creditLimit').updateValueAndValidity;
                    }
                });
        }
    ...
    ngAfterViewInit(): void {
        ...
        this.searchSalesman.valueChanges
            .pipe(
                takeUntil(this._unSubs$),
                debounceTime(500),
                distinctUntilChanged(),
                tap((value: string) => {
                    this.store.dispatch(
                        DropdownActions.fetchDropdownSalesRepRequest({
                            search: [{ fieldName: 'keyword', keyword: value  }],
                        })
                    );
                })
            )
    ...
    ngOnDestroy(): void {
        ...
        this.isCheckingKtp$.next(false);
        this.isCheckingKtp$.complete();
        ...
        // Reset sales rep state
        this.store.dispatch(DropdownActions.resetSalesRepState());
    ...
    get salesRep(): any {
        return this.form.get('storeInfo.salesRep').value;
    }
    ...
    openSalesman(): void {
        this.salesSelect.open();
    }

    closeSalesman(): void {
        this.salesSelect.close();
    }

    onSalesRepOptionChange(ev: MatOptionSelectionChange): void {
        if (ev.isUserInput) {
            if (this.pageType === 'edit') {
                            ...
                            if (response.storeId) {
                                ...
                                if (!response.availability) {
                                    return {
                                        notAvailable: {
                                            message:
                                                'This phone number is already registered by this supplier.',
                                            refValues: [value],
                                        },
                                    };
                                }
    ...
    onStoreTypeSelectionChanged($event: SelectedTree): void {
        ...
        this.selectedStoreType$.next({
            ...
            current: $event.selectedString
                ? $event.selectedString
                : this.selectedStoreType$.value.lastSaved,
        });
    }
    ...
    onStoreGroupSelectionChanged($event: SelectedTree): void {
        ...
        this.selectedStoreGroup$.next({
            ...
            current: $event.selectedString
                ? $event.selectedString
                : this.selectedStoreGroup$.value.lastSaved,
        });
    }
    ...
    onStoreChannelSelectionChanged($event: SelectedTree): void {
        ...
        this.selectedStoreChannel$.next({
            ...
            current: $event.selectedString
                ? $event.selectedString
                : this.selectedStoreChannel$.value.lastSaved,
        });
    }
    ...
    onStoreClusterSelectionChanged($event: SelectedTree): void {
        ...
        this.selectedStoreCluster$.next({
            ...
            current: $event.selectedString
                ? $event.selectedString
                : this.selectedStoreCluster$.value.lastSaved,
        });
    }
    ...
    displaySalesRep(sources: Array<SalesRep>, selectedSources: string): any {
        const idx = sources.findIndex((s) => {
            if (!!selectedSources) return s.userId === selectedSources;
            else return false;
        });

        if (idx !== -1) {
            if (!!sources[idx].user.fullName) return sources[idx].user.fullName;
            else sources[idx].user.name;
        }
        ...
        return (control: AbstractControl): { [key: string]: any } | null => {
            ...
            if (!isWarehouseSelected && this.availableWarehouses.length > 1) {
                return {
                    multipleWarehouses: {
                        message:
                            'This store is within the coverage area of multiple warehouses. Please choose ONE of the listed warehouses.',
                        value: control.value,
                    },
                };
            }
            ...
            if (file) {
                switch (type) {
                    ...
                    case 'owner-identity-selfie-photo':
                        {
                            const photoField = this.form.get('profileInfo.identityPhotoSelfie');
                    ...
                    case 'store-tax-photo':
                        {
                            const photoField = this.form.get('storeInfo.taxPhoto');
                    ...
                    case 'store-photo':
                        {
                            const photoField = this.form.get('storeInfo.photo');
    ...
    onKeyup(ev: KeyboardEvent, field: string): void {
        switch (field) {
            ...
            case 'storeId':
                ...
                this._timer[field] = setTimeout(() => {
                    this.store.dispatch(
                        StoreActions.checkStoreExternalIdRequest({
                            externalId: (ev.target as any).value,
                        })
                    );
                }, 350);
                break;
        ...
        fromEvent(this.autoDistrict.panel.nativeElement, 'scroll')
            .pipe(
                ...
                filter(
                    ({ scrollTop, scrollHeight, elHeight }) => {
                        return scrollHeight <= scrollTop + elHeight + 96;
                    }
                ),
                tap(({ scrollTop, scrollHeight, elHeight }) =>
                    HelperService.debug(
                        '[MerchantFromComponent tap] onOpenAutocomplete fromEvent',
                        {
                            scrollTop,
                            scrollX,
                            scrollHeight,
            ...
            .subscribe({
                next: ([{ scrollTop, scrollHeight, elHeight }, skip, total]) => {
                    const atBottom = scrollHeight <= (scrollTop + elHeight) + 96;

                    HelperService.debug(
                        '[MerchantFromComponent next] onOpenAutocomplete fromEvent',
                        {
                            scrollTop,
                            scrollX,
                            scrollHeight,
        ...
        if (urbanId) {
            ...
            of(null)
                .pipe(
                    ...
                    switchMap<[null, UserSupplier], Observable<Array<WarehouseDropdown>>>(
                        ([_, userSupplier]) => {
                ...
                .subscribe({
                    next: (response) => {
                        ...
                        if (this.availableWarehouses.length === 1) {
                            ...
                            this.form
                                .get('storeInfo.address.warehouse')
                                .setValue(this.availableWarehouses[0].id);
                        } else {
                        ...
                        this.form.get('storeInfo.address.warehouse').updateValueAndValidity;
                    },
                    error: (err) => {
                        this.isWarehouseLoading$ = false;
        ...
        if (data && data.length > 0) {
            for (const [idx, row] of data.entries()) {
                if (row.id) {
                    ...
                    this.tempInvoiceGroupName = this.tempInvoiceGroupName.filter(
                        (el, index, arr) => arr.indexOf(el) === index
                    );

                    if (this.tempInvoiceGroupName.length > 0) {
                        ...
                        this.checkDataRemove(data);
                    } else {
                        this.formCreditLimits.at(idx).get('allowCreditLimit').reset();

                        this.formCreditLimits.at(idx).get('allowCreditLimit').patchValue(false);

                        this.formCreditLimits.at(idx).get('invoiceGroup').patchValue(row.id);
    ...
    checkDataRemove(dataInvoice) {
        if (dataInvoice.length > 1) {
            for (
                let idx = 0;
                idx < this.form.get('storeInfo.payment.creditLimit').value.length;
                idx++
            ) {
                if (
                    !this.form
                        .get('storeInfo.payment.creditLimit')
                        .value[idx].hasOwnProperty('invoiceGroup') ||
                    this.form.get('storeInfo.payment.creditLimit').value[idx].invoiceGroup === null
                ) {
                    this.formCreditLimits.removeAt(idx);
                }
            }
            this.formCreditLimits.updateValueAndValidity();
        } else if (dataInvoice.length === 1) {
            let dataCrLimit = this.form
                .get('storeInfo.payment.creditLimit')
                .value.filter((element) => {
                    if (element.invoiceGroup) {
                        return element;
                    }
                });

            //remove data if null invoicegroup
            for (let idx = 0; idx < this.formCreditLimits.value.length; idx++) {
                if (this.formCreditLimits.value[idx].invoiceGroup === null) {
                    this.formCreditLimits.removeAt(idx);
                    this.formCreditLimits.updateValueAndValidity();
                }
            }

            //checking again jika mm lebih dari 0
            if (dataCrLimit.length > 0) {
                if (this.formCreditLimits.value.length !== dataCrLimit.length) {
                    //filter data again
                    for (let idx = 0; idx < this.formCreditLimits.value.length; idx++) {
                        if (
                            !this.formCreditLimits.value[idx].hasOwnProperty('invoiceGroup') ||
                            this.formCreditLimits.value[idx].invoiceGroup === null
                        ) {
                            this.formCreditLimits.removeAt(idx);
                            this.formCreditLimits.updateValueAndValidity();
                        }
            ...
            storeInfo: this.formBuilder.group({
                ...
                address: this.formBuilder.group({
                    ...
                    warehouse: [
                        ...
                        [
                            RxwebValidators.required({
                                conditionalExpression: (x: Urban | string) =>
                                    !!x && this.availableWarehouses.length === 0,
                                message: this._$errorMessage.getErrorMessageNonState(
                                    'default',
                                    'required'
                                ),
            ...
            .subscribe(([_, invoiceGroups, data]) => {
                if (this.pageType === 'new') {
                    ...
                    if (data) {
                        if (
                            this.formBackup &&
                            this.formBackup.storeInfo.address.urban !== '' &&
                            data['outerStore']['urbanId'] !==
                                this.formBackup.storeInfo.address.urban.id
                        ) {
                            this.requestWarehouse(
                                this.formBackup.storeInfo.address.urban.id,
                                false
                            );
                            this.statusChangesWh = true;
                        } else {
                            this.requestWarehouse(+data['outerStore']['urbanId'] || null, false);
                        }
                        if (data['outerStore']['warehouseId'] && invoiceGroups.length < 1) {
                            this.dataOuterWh = data['outerStore']['warehouseId'];
                            this.store.dispatch(
                                DropdownActions.fetchDropdownInvoiceGroupWhSupRequest({
                                    warehouseId: data['outerStore']['warehouseId'],
                                })
                            );
                        }
                        ...
                        this.form
                            ...
                            .subscribe((v) => {
                                if (
                                    (this.form.get('storeInfo.address').value.warehouse ||
                                        !this.form.get('storeInfo.address').value.warehouse) &&
                                    this.form.get('storeInfo.address').value.warehouse !== v && !data['outerStore']['warehouseId']
                                ) {
                                    this.statusChangesWh = true;
                                    this.dataOuterWh = v;
                                    if (v !== null) {
                                        ...
                                        this.store.dispatch(
                                            DropdownActions.fetchDropdownInvoiceGroupWhSupRequest({
                                                warehouseId: this.dataOuterWh,
                                            })
                                        );
                                        data['outerStore']['warehouseId'] = v;

                                        this.form.get('storeInfo.address.warehouse')
                                            .updateValueAndValidity;
                                        this.form.get('storeInfo.payment.creditLimit')
                                            .updateValueAndValidity;
                                    }
                                }
                            });
                        ...
                        if (invoiceGroups.length > 0) {
                            ...
                            if (
                                ...
                                this.restoreFormDataWithoutAddress(data, invoiceGroups);
                            } else {
                                this.restoreFormData(data, invoiceGroups);
                            }
                        } 
                    }
                }
    ...
    validateAddress(): ValidatorFn {
        return (control: AbstractControl): ValidationErrors | null => {
            const regex =
                /(?=.*jalan|jl|jln|komp|kompl|perum|perumahan|ruko|kampung|kp|lt|lantai)((.*[1-9]+).*)|((.*[1-9]+).*)(?=.*jalan|jl|jln|komp|kompl|perum|perumahan|ruko|kampung|kp|lt|lantai)/gi;
            if (control.value && control.value.length > 0 && !regex.test(control.value)) {
                return {
                    error: {
                        message: 'Lengkapi alamat dengan jalan / komplek / perumahan dan nomor',
                    },
                };
            }
            return null;
    ...
    private restoreFormDataWithoutAddress(
        data: SupplierStore,
        invoice?: Array<InvoiceGroup>
    ): void {
        ...
        if (data['store']['creditLimitStores'] && data['store']['creditLimitStores'].length > 0) {
            ...
            let availableCreditLimitStores: Array<any> = this.form
                .get('storeInfo.payment.creditLimit')
                .value.filter((element) => {
                    if (element.invoiceGroup) {
                        return element;
            ...
            for (const [idx, row] of creditLimitStores.entries()) {
                if (typeof row.allowCreditLimit === 'boolean') {
                    const foundIdx = availableCreditLimitStores.findIndex(
                        (cl) => cl.invoiceGroup === row.invoiceGroupId
                    );

                    if (foundIdx >= 0) {
                        this.tempInvoiceGroupName[foundIdx] = !row.invoiceGroup
                            ? '-'
                            : row.invoiceGroup.name;

                        const allowCreditLimit = !!row.allowCreditLimit;

                        if (
                            this.formCreditLimits.value.length !== availableCreditLimitStores.length
                        ) {
                            //remove data if null invoicegroup
                            for (let idx = 0; idx < this.formCreditLimits.value.length; idx++) {
                                if (this.formCreditLimits.value[idx].invoiceGroup === null) {
                        ...
                        if (
                            ...
                            if (this.statusChangesWh !== true) {
                                this.patchValueCreditLimit(foundIdx, allowCreditLimit, row);
                                this.form.get('storeInfo.payment.creditLimit')
                                    .updateValueAndValidity;
                            }
                        }
    ...
    validateKtp(): AsyncValidatorFn {
        return (control: AbstractControl): Observable<ValidationErrors | null> => {
            this.isCheckingKtp$.next(true);
            return this._$helper.validateKTP(control.value).pipe(
                map((result: { [key: string]: { [key: string]: string } }) => {
                    this.isCheckingKtp$.next(false);
                    return result;
                }),
                catchError(() => {
    ...
    private restoreFormData(data: SupplierStore, invoice?: Array<InvoiceGroup>): void {
        this.currentStoreId = `Current Store ID: ${data['outerStore']['externalId']}`;
        ...
        if (!!data['outerStore']['assignedSaleRep'] && !!data['outerStore']['assignedSaleRep']['id']) {
            this.form.patchValue({
                storeInfo: {
                    salesRep: data['outerStore']['assignedSaleRep']['id']
                }
            })
        }

        if (!!data['outerStore']['taxStatus']) {
            this.form.patchValue({
                storeInfo: {
                    taxStatus: data['outerStore']['taxStatus']
                }
            })
        }
        ...
        if (data['store']['creditLimitStores'] && data['store']['creditLimitStores'].length > 0) {
            const creditLimitStores = data['store']['creditLimitStores'] as Array<CreditLimitStore>;
            // let availableCreditLimitStores: Array<any> = this.formCreditLimits.getRawValue();
            let availableCreditLimitStores: Array<any> = this.form
                .get('storeInfo.payment.creditLimit')
                .value.filter((element) => {
                    if (element.invoiceGroup) {
    ...
    patchValueCreditLimit(foundIdx, allowCreditLimit, row) {
    ...
    private createFormListener(): void {
        (this.form.statusChanges as Observable<FormStatus>)
            .pipe(
                ...
                debounceTime(300),
                tap((status) =>
                    HelperService.debug('PERIOD TARGET PROMO FORM STATUS CHANGED', status)
                ),
                takeUntil(this._unSubs$)
            )
            .subscribe();
        ...
        this.form.valueChanges
            .pipe(
                ...
                debounceTime(300),
                tap((value) =>
                    HelperService.debug('PERIOD TARGET PROMO FORM VALUE CHANGED', {
                        form: this.form,
                        value,
                    })
                ),
            ...
            .subscribe((val) => {
                this.formBackup = this.form.value;
            });
    }
    ...
    private onSubmit(id?: string): void {
        ...
        if (body.supplierId) {
            ...
            HelperService.debug("BODY TO PAYLOAD", body)
            const payload = {
                ...
                approvalStatus: body.storeInfo.approvalStatus
                    ? body.storeInfo.approvalStatus
                    : 'guest',
                urbanId: urban.id,
                user: {
                ...
                warehouseId: body.storeInfo.address.warehouse,
                type: {
                ...
                taxStatus: body.storeInfo.taxStatus,
                salesRepId: body.storeInfo.salesRep
            };
            ...
            if (
                body.profileInfo.taxPhoto !== body.profileInfo.oldTaxPhoto &&
            ...
            if (
                body.profileInfo.identityPhoto !== body.profileInfo.oldIdentityPhoto &&
            ...
            if (
                body.profileInfo.identityPhotoSelfie !== body.profileInfo.oldIdentityPhotoSelfie &&
            ...
            if (
                body.storeInfo.taxPhoto !== body.storeInfo.oldTaxPhoto &&
            ...
            if (
                body.storeInfo.photo !== body.storeInfo.oldPhoto &&
            ...
            if (!body.storeInfo.taxStatus) {
                delete payload.taxStatus;
            }

            if (!body.storeInfo.salesRep) {
            ...
            if (uploadPhotos.length === 0) {
                if (!id) {
                    ...
                    this.store.dispatch(
                        StoreActions.updateStoreRequest({ payload: { body: payload, id } })
                    );
                }
            } else {
                ...
                of<UploadPhotoApiPayload>(...uploadPhotos)
                    ...
                    .subscribe({
                        next: (value) => {
                            HelperService.debug(
                                '[STORE FORM] UPLOAD PHOTO SEQUENTIAL SUCCESS',
                                value
                            );
                        },
                        error: (err) => {
                        ...
                        complete: () => {
                            ...
                            setTimeout(() => {
                                ...
                                if (!id) {
                                    this.store.dispatch(
                                        StoreActions.createStoreRequest({ payload })
                                    );
                                } else {
                                    this.store.dispatch(
