...
export class StoreDetailPageComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    tabIndex: number = 0;
    reRenderEmployeeComponent: boolean = false;
    ...
    private initRefreshStatus(): void {
        ...
        this.store
            ...
            .pipe(withLatestFrom(this.selectedSupplierStore$), takeUntil(this.subs$))
    ...
    onSelectedTab(tabChangeEvent: MatTabChangeEvent): void {
        switch (tabChangeEvent.index) {
            case 0:
                ...
                break;
            case 1:
                if (this.bulkPriceSettingStatus === 'group_pricing') {
                    this.section = 'group_price';
                    this.router.navigateByUrl(
                        `/pages/account/stores/${this.route.snapshot.params.id}/detail/group_price`,
                        { replaceUrl: true }
                    );
                } else {
    ...
    ngOnInit(): void {
        ...
        this.isEditLocation$ = this.store
            .select(StoreSelectors.getIsEditLocation)
            .pipe(takeUntil(this.subs$));

        // Mendapatkan supplier store yang terpilih.
        this.selectedSupplierStore$ = this.store
            .select(StoreSelectors.getSelectedSupplierStore)
            .pipe(takeUntil(this.subs$));

        this.selectedSupplierStore$.subscribe((item) => {
            localStorage.setItem('ssc-store-detail', JSON.stringify(item));
        });
                                    ...
                                    tap(({ type, group, channel }) => {
                                        ...
                                        const newSupplierStore: SupplierStore = {
                                            ...
                                            outerStore: {
                                                ...
                                                selectedStoreType:
                                                    String(type.text || '').replace(
                                                        'class="my-12"',
                                                        ''
                                                    ) || '-',
                                                selectedStoreGroup:
                                                    String(group.text || '').replace(
                                                        'class="my-12"',
                                                        ''
                                                    ) || '-',
                                                selectedStoreChannel:
                                                    String(channel.text || '').replace(
                                                        'class="my-12"',
                                                        ''
                                                    ) || '-',
                                                // selectedStoreCluster: cluster.text || '-',
                                                rejectedFieldsString:
                                                    this.getRejectedFields(supplierStore),
                                            },
                                        };

                                        // Mengganti Supplier Store yang terpilih dengan yang terbaru (yang sudah ada data Store Classification-nya)
                                        this.store.dispatch(
                                            StoreActions.selectSupplierStore({
                                                payload: newSupplierStore,
                                            })
                                        );
                                    })
                                )
    ...
    ngAfterViewInit(): void {
        ...
        if (tab) {
            this.section = tab;
            this.reRenderEmployeeComponent = false;
            switch (this.section) {
                case 'store':
                    this.tabIndex = 0;
                    break;
                case 'employee':
                    this.tabIndex = 1;
                    this.reRenderEmployeeComponent = true;
                    break;
                case 'location':
                    this.tabIndex = 2;
                    break;
