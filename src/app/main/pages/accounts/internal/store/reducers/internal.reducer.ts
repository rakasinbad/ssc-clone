...
import { InternalEmployeeDetails, InternalWarehouses } from '../../models';
...
const internalReducer = createReducer(
    ...
    on(
        ...
        InternalActions.fetchInternalWarehousesRequest,
        state => ({
            ...state,
            isLoading: true
        })
    ...
    on(InternalActions.fetchInternalWarehousesSuccess, (state, { payload }) => ({
        ...state,
        isLoading: false,
        isRefresh: undefined,
        internalWarehouses: payload,
        errors: adapterError.removeOne('fetchInternalWarehousesFailure', state.errors)
    })),
