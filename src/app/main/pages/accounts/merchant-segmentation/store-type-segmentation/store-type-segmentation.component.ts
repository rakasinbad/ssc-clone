...
import { PayloadStoreType, PayloadStoreTypePatch, StoreType, ISelectedSegment } from '../models';
...
export class StoreTypeSegmentationComponent implements OnInit, OnDestroy {
    ...
    private _items: Array<StoreType> = [];
        ...
        if (!isSelector) {
            ...
            const branchIds =
                segments[idx].branches && segments[idx].branches.length > 0
                    ? segments[idx].branches.map((branch: { id: any; }) => branch.id)
                    : [];
    ...
    generateSelected(): string {
        ...
        const segments = formValue.segments
            .map((segmentItem: { selectedId: any; branches: any[]; }) => {
                if (
                    ...
                    const branches = segmentItem.branches
                        .filter((branchItem: { id: any; }) => branchItem.id === segmentItem.selectedId)
                        .map((branchItem: { name: any; }) => branchItem.name);
    ...
    private _onSubmit(segmentIdx: number, branchIdx: number): void {
        ...
        const parentIdx = body.segments[segmentIdx].branches.findIndex(
            (            branch: { id: any; }) => branch.id === parentId
        );
        ...
        if (Object.keys(payload).length > 0) {
            this.store.dispatch(
                StoreTypeActions.updateStoreTypeRequest({
                    payload: { body: new PayloadStoreTypePatch(payload), id: formValue.id }
                })
            );
        } else {
            this.form
