...
import { PayloadStoreChannel, PayloadStoreChannelPatch, StoreChannel, ISelectedSegment } from '../models';
...
export class StoreChannelSegmentationComponent implements OnInit, OnDestroy {
    ...
    private _items: Array<StoreChannel> = [];
    ...
    handleEvent(segmentIdx: number, branchIdx: number): void {
        ...
        // this._resetSegment(segmentIdx); // fix reset segment (level auto close)
    ...
    private _initPage(lifeCycle?: LifecyclePlatform): void {
        switch (lifeCycle) {
            ...
            default:
                ...
                this.store
                    .select(StoreChannelSelectors.getSelectedId)
                    .pipe(
                        filter(v => !!v),
                        takeUntil(this._unSubs$)
                    )
                ...
                this.store
                    .select(StoreChannelSelectors.getDeactiveItem)
                    .pipe(
                        filter(v => !!v),
                        takeUntil(this._unSubs$)
                    )
    ...
    private _onRefreshHierarchy(): void {
        this.store.select(StoreChannelSelectors.getIsError).subscribe(isError => {
            if (isError){
        ...
        if (Object.keys(payload).length > 0) {
            this.store.dispatch(
                StoreChannelActions.updateStoreChannelRequest({
                    payload: { body: new PayloadStoreChannelPatch(payload), id: formValue.id }
                })
            );
        } else {
            this.form
