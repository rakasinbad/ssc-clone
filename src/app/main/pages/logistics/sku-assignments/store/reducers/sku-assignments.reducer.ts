...
// New SKU's Warehouse
interface NewCatalogueState extends EntityState<any> {
    selectedIds: Array<string>;
}
export const adapterNewCatalogue: EntityAdapter<any> = createEntityAdapter<any>({
...
export const reducer = createReducer(
    ...
    on(SkuAssignmentsActions.addSelectedCatalogues, (state, { payload }) => {
        const newCatalogues = (payload as Array<any>).map(store => {
            const nC = new store;
            nC.isSelected = true;
            return nC;
        });
    ...
    on(SkuAssignmentsActions.markCatalogueAsRemovedFromWarehouse, (state, { payload }) => {
        const newStore = new state.newSku.entities[payload];
    ...
    on(SkuAssignmentsActions.markCataloguesAsRemovedFromWarehouse, (state, { payload }) => {
        const newStore: Array<any> = [];

        for (const storeId of payload) {
            const _store = new state.newSku.entities[storeId];
