...
const reducer = createReducer<State>(
    initialState,
    on(StockManagementReasonActions.fetchStockManagementReasonFailure, (state, { payload }) => {
        return adapter.upsertOne(payload, state);
    }),
    on(StockManagementReasonActions.fetchStockManagementReasonSuccess, state => {
        return adapter.removeOne('fetchStockManagementReasonFailure', state);
    }),
