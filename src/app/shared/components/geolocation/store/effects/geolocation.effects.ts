...
import { GeolocationActions, GeolocationFailureActionNames } from '../actions';
            ...
            this.actions$.pipe(
                ...
                tap(({ payload }) => {
                    if (environment.environment !== 'production') {
                        this.notice$.open(
                            'Failed to fetch provinces.<br/><br/>Code: ' +
                                payload.id +
                                '<br/>Reason: ' +
                                payload.errors,
                            'error',
        ...
        () =>
            this.actions$.pipe(
                ...
                tap(({ payload }) => {
                    if (environment.environment !== 'production') {
                        this.notice$.open(
                            'Failed to fetch cities.<br/><br/>Code: ' +
                                payload.id +
                                '<br/>Reason: ' +
                                payload.errors,
                            'error',
    ...
    fetchUrbansRequest$ = createEffect(() =>
        this.actions$.pipe(
            ...
            withLatestFrom(
                ...
                this.locationStore.select(GeolocationSelectors.getSelectedDistrict)
            ),
            // Mengubah jenis Observable yang menjadi nilai baliknya. (Harus berbentuk Action-nya NgRx)
            switchMap(
        ...
        () =>
            this.actions$.pipe(
                ...
                tap(({ payload }) => {
                    if (environment.environment !== 'production') {
                        this.notice$.open(
                            'Failed to fetch urbans.<br/><br/>Code: ' +
                                payload.id +
                                '<br/>Reason: ' +
                                payload.errors,
                            'error',
