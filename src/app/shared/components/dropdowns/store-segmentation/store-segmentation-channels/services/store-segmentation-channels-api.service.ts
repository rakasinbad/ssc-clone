 ...
 *
 * @export
 * @class StoreSegmentationChannelsApiService
 */
@Injectable({
    providedIn: 'root'
})
export class StoreSegmentationChannelsApiService {
    private _url: string;
    ...
    find<T>(params: IQueryParams): Observable<T> {
        ...
        if (!params['supplierId'] && !params['noSupplierId']) {
            throw new Error('ERR_STORE_SEGMENTATION_CHANNELS_REQUIRE_SUPPLIERID');
        }
