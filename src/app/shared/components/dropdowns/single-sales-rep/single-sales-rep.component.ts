...
import { tap, debounceTime, withLatestFrom, filter, takeUntil, distinctUntilChanged, take, catchError, switchMap } from 'rxjs/operators';
import { SalesRep as Entity } from './models';
import {
    SalesRepApiService as EntitiesApiService,
    SingleSalesRepDropdownService as EntitiesService
} from './services';
...
@Component({
    selector: 'select-single-sales-rep',
    templateUrl: './single-sales-rep.component.html',
    styleUrls: ['./single-sales-rep.component.scss'],
    animations: fuseAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.Default
})
export class SingleSalesRepDropdownComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
    ...
    @Input('placeholder') placeholder: string = 'Search Sales Rep.';
    ...
    @ViewChild('selectEntity', { static: false }) selectEntity: TemplateRef<MultipleSelectionComponent>;
    ...
    ngOnInit(): void {
        ...
        this.entitiesService.getSelectedSalesRep().pipe(
            tap(value => HelperService.debug('SINGLE SALES REP GET SELECTED SALES REP', value)),
            takeUntil(this.subs$)
        ).subscribe(warehouse => {
            if (warehouse) {
    ...
    ngOnDestroy(): void {
        ...
        this.entitiesService.selectSalesRep(null);
