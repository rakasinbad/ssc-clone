...
import { Brand as Entity } from './models';
import { BrandsApiService as EntitiesApiService } from './services';
...
export class BrandsDropdownComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
    ...
    // tslint:disable-next-line: no-inferrable-types
    @Input() placeholder: string = 'Choose Brand';
        ...
        of(null).pipe(
            ...
            next: (response) => {
                ...
                if (Array.isArray(response)) {
                    ...
                    addedAvailableEntities = (response as Array<Entity>).map(d => ({ id: d.id, label: d.name, group: 'brands' }));
                } else {
                    addedRawAvailableEntities = response.data;
                    addedAvailableEntities = (response.data as Array<Entity>).map(d => ({ id: d.id, label: d.name, group: 'brands' }));
                }
