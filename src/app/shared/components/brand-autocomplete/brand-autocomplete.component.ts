...
import { BrandAutocomplete } from './models';
...
export class BrandAutocompleteComponent implements OnChanges, OnInit, OnDestroy {
    ...
    readonly placeholder: string = 'Brand';

    collections$: Observable<BrandAutocomplete[]>;
    ...
    onOpenedAutocomplete(): void {
        ...
        this.authFacade.getUserSupplier$
            ...
            .subscribe({
                ...
                complete: () =>
                    HelperService.debug(
                        '[BrandAutocompleteComponent] onOpenedAutocomplete complete'
                    ),
            });
    }

    onScrollToBottom(): void {
        combineLatest([this.BrandService.totalCollections$, this.BrandService.total$])
            .pipe(
    ...
    private _initCollections(keyword: string, supplierId: string): void {
        HelperService.debug('[BrandAutocompleteComponent] _initCollections', {
            keyword,
        });
        ...
        this.BrandService.getWithQuery(params);
    }

    private _initScrollCollections(skip: number = 0, keyword: string, supplierId: string): void {
        HelperService.debug('[BrandAutocompleteComponent] _initScrollCollections', {
            skip,
        });
    ...
    private _convertKeyword(value: any): string {
        return value && value.hasOwnProperty('id') && typeof value !== 'string'
            ? ((value as unknown) as BrandAutocomplete).label
            : value;
    }
}
