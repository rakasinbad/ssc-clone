...
export class ExportsEffects {
        ...
        this.actions$.pipe(
            ...
            switchMap(
                ([queryParams, authState]: [
                    IQueryParams & ExportConfiguration,
                    TNullable<Auth>
                ]) => {
                    return of([queryParams, authState] as [
                        IQueryParams & ExportConfiguration,
                        TNullable<Auth>
                    ]).pipe(
                        // Pemeriksaan error.
                        ...
                        exhaustMap(
                            ([_, __]: [IQueryParams & ExportConfiguration, TNullable<Auth>]) => {
                                    ...
                                    return dialogRef.afterClosed().pipe(
                                        map(
                                            (
                                                val:
                                                    | undefined
                                                    | { page: string; payload: ExportFormData }
                                            ) => {
                                                if (!val) {
                                                    return val;
                                                } else {
                            ...
                            ([query, __]: [
                                ...
                                if (!authState) {
                                    return this.helper$.decodeUserToken().pipe(
                                        ...
                                        catchError((err) =>
                                            this.sendErrorToState(err, 'startExportFailure')
                                        )
                                    );
                                } else {
        ...
        () =>
            this.actions$.pipe(
                ...
                tap((error) => {
                    ...
                    this.notice$.open(
                        `Something wrong with our web while preparing export dialog. Please contact Sinbad Team. Error code: ${error.id}`,
                        'error',
                        noticeSetting
                    );
                })
    ...
    fetchExportLogsRequest$ = createEffect(() =>
        this.actions$.pipe(
            ...
            // Mengubah jenis Observable yang menjadi nilai baliknya. (Harus berbentuk Action-nya NgRx)
            switchMap(
                ([queryParams, exportPage, authState]: [IQueryParams, string, TNullable<Auth>]) => {
                    // Jika tidak ada data supplier-nya user dari state.
                    if (!authState) {
                        return this.helper$.decodeUserToken().pipe(
                            map(this.checkUserSupplier),
                            ...
                            switchMap<[User, string, IQueryParams], Observable<AnyAction>>(
                                this.processFetchExportLogsRequest
                            ),
                            catchError((err) =>
                                this.sendErrorToState(err, 'fetchExportLogsFailure')
                            )
                        );
                    } else {
                        return of(authState.user).pipe(
    ...
    processFetchExportLogsRequest = ([userData, exportPage, queryParams]: [
        User,
        ExportConfiguration['page'],
        IQueryParams
    ]): Observable<AnyAction> => {
        ...
        return this.exportsApiService.findExportLogs<IPaginatedResponse<Export>>(newQuery).pipe(
            catchOffline(),
            switchMap((response) => {
    ...
    continueToStartExport = ([_, queryParams]: [
        User,
        IQueryParams & ExportConfiguration & { formData: ExportFormData }
    ]): Observable<AnyAction> => {
